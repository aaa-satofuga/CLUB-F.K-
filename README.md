 ...
 <body><!-- Background Stars -->
  ...
  <nav class="side-menu" id="sideMenu">
   <div class="modal-close" onclick="closeMenu()" style="position: absolute; top: 20px; right: 20px; z-index: 10;">
    ×
   </div><a class="menu-item" onclick="scrollToSection('cast')">👤 キャスト</a> <a class="menu-item" onclick="scrollToSection('news')">📰 ニュース</a> <a class="menu-item" onclick="scrollToSection('album')">📷 アルバム</a> <a class="menu-item" onclick="scrollToSection('store')">������ 店舗紹介</a> <a class="menu-item" onclick="scrollToSection('price')">💰 料金システム</a> <a class="menu-item" onclick="scrollToSection('recruit')">💼 求人募集</a>
  </nav><!-- Main -->
  <main class="main"><!-- Hero -->
   <section class="hero">
    <div>
     <h1 class="hero-title" id="heroTitle">CLUB F.K</h1>
     <p class="hero-subtitle" id="heroSubtitle">最高な時間を貴方と共に</p>
    </div>
   ...
   <section class="section" id="cast">
    <h2 class="section-title">キャスト紹介</h2>
    <div class="grid" id="castGrid"></div><button class="add-btn" id="addCast" style="display: none;">+ キャストを���加</button>
   </section><!-- News -->
   <section class="section" id="news">
    <h2 class="section-title">ニュース</h2>
    <div class="news-album-grid" id="newsGrid"></div>
    <div class="pagination" id="newsPagination" style="display: none;"><button class="pagination-btn" id="newsPrev">前</button> <span class="pagination-info" id="newsPageInfo"></span> <button class="pagination-btn" id="newsNext">後</button>
    </div><button class="add-btn" id="addNews" style="display: none;">+ ニュースを追加</button>
   </section><!-- Album -->
   <section class="section" id="album">
    <h2 class="section-title">アルバム</h2>
    <div class="news-album-grid" id="albumGrid"></div>
    <div class="pagination" id="albumPagination" style="display: none;"><button class="pagination-btn" id="albumPrev">前</button> <span class="pagination-info" id="albumPageInfo"></span> <button class="pagination-btn" id="albumNext">後</button>
    </div><button class="add-btn" id="addAlbum" style="display: none;">+ アルバムを追加</button>
   </section><!-- Store -->
   <section class="section" id="store">
    <h2 class="section-title">店舗紹介</h2>
    <div id="storeContent"></div><button class="add-btn" id="addStore" style="display: none;">+ 項目を追加</button>
   </section><!-- Price -->
   <section class="section" id="price">
    <h2 class="section-title">料金システム</h2>
    <div id="priceContent"></div><button class="add-btn" id="addPrice" style="display: none;">+ 項目を追加</button>
   </section><!-- Recruit -->
   <section class="section" id="recruit">
    <h2 class="section-title">求人募集</h2>
    <div id="recruitContent"></div><button class="add-btn" id="addRecruit" style="display: none;">+ 項目を追加</button>
   </section>
  ...
  <div class="password-modal" id="passwordModal">
   <div class="password-box">
    ...
    <form id="passwordForm"><input type="password" class="password-input" id="passwordInput" placeholder="***" maxlength="3" required>
     ...
     <div class="password-buttons"><button type="submit" class="password-btn password-btn-submit">確認</button> <button type="button" class="password-btn password-btn-cancel" onclick="closePasswordModal()">キャンセル</button>
     </div>
    </form>
   </div>
  </div><!-- Form Modal -->
  <div class="modal" id="formModal">
   <div class="modal-content">
    ...
    <form id="dataForm">
     ...
     <div class="form-buttons" id="formButtons"><button type="submit" class="form-btn form-btn-submit" id="submitBtn">保存</button> <button type="button" class="form-btn form-btn-cancel" onclick="closeModal()">キャンセル</button>
     </div>
    </form>
   </div>
  </div>
  <script>
    ...
    function openShowcaseSizeEdit(item, position) {
      ...
      tempModal.innerHTML = `
        <div class="password-box" style="max-width: 500px;">
          <div class="password-title">サイズ変更</div>
          <div style="margin-bottom: 20px;">
            <label style="display: block; color: #ffd700; margin-bottom: 10px; font-weight: 600; font-size: 15px;">幅 (px)</label>
          ...
          <div style="margin-bottom: 20px;">
            <label style="display: block; color: #ffd700; margin-bottom: 10px; font-weight: 600; font-size: 15px;">高さ (px)</label>
          ...
          <div class="password-buttons">
            <button class="password-btn password-btn-submit" id="confirmSize">���存</button>
            <button class="password-btn password-btn-cancel" id="cancelSize">キャンセル</button>
          </div>
        </div>
      `;
        ...
        if (item && item.showcaseImage) {
          ...
          if (editMode) {
            ...
            del.onclick = async (e) => {
              ...
              tempModal.innerHTML = `
                <div class="password-box">
                  <div class="password-title">確認</div>
                  <p style="color: #fff; margin-bottom: 30px;">このショー��ース画像を削除しますか？</p>
                  <div class="password-buttons">
                    <button class="password-btn password-btn-submit" id="confirmDelete">削除</button>
                    <button class="password-btn password-btn-cancel" id="cancelDelete">キャンセル</button>
                  </div>
                </div>
              `;
              document.body.appendChild(tempModal);
    ...
    function openCastDetail(item) {
      ...
      if (item.instagram) {
        snsHtml += `<a href="${item.instagram}" target="_blank" rel="noopener noreferrer" class="sns-icon sns-instagram">📷</a>`;
      }
      if (item.tiktok) {
        snsHtml += `<a href="${item.tiktok}" target="_blank" rel="noopener noreferrer" class="sns-icon sns-tiktok">🎵</a>`;
      }
      if (item.x) {
        snsHtml += `<a href="${item.x}" target="_blank" rel="noopener noreferrer" class="sns-icon sns-x">𝕏</a>`;
      }
      if (item.hoshos) {
      ...
      content.innerHTML = `
          ...
          <div class="cast-detail-info">
            ...
            <div class="cast-detail-fields-row">
              <div class="cast-detail-field">
                <span class="cast-detail-label">誕生日:</span>
                <span class="cast-detail-value" id="castDetailBirthday">${item.birthday || '未設定'}</span>
              </div>
              <div class="cast-detail-field">
                <span class="cast-detail-label">MBTI:</span>
                <span class="cast-detail-value" id="castDetailMbti">${item.mbti || '未設定'}</span>
              </div>
            </div>
            ${snsHtml}
          </div>
        ...
        <div class="cast-detail-gallery">
          <div class="cast-detail-gallery-title">ギャラリー</div>
    ...
    function openGalleryEdit(item, index) {
      ...
      document.getElementById('formTitle').textContent = 'ギャラリー画像を編集';
      document.getElementById('formFields').innerHTML = `
        <div class="form-group">
          <label class="form-label" for="galleryImageUrl">画像URL *</label>
    ...
    function openGalleryAdd(item) {
      ...
      document.getElementById('formTitle').textContent = 'ギャラリー画像を追加';
      document.getElementById('formFields').innerHTML = `
        <div class="form-group">
          <label class="form-label" for="galleryImageUrl">画像URL *</label>
    ...
    function openAlbumLightbox(item) {
      ...
      if (albumUrls.length === 0) {
        ...
        message.textContent = 'このアルバムに画像がありません';
    ...
    function renderStore() {
      ...
      addressText.innerHTML = `<strong>📍 場所:</strong><br>${addressItem?.address || '札幌���中央区南5条西6丁目第3エトワールビル5階'}`;
      ...
      mapLink.textContent = '🗺️ Googleマップで見る';
      ...
      snsSection.innerHTML = `
        <div class="sns-section-title">SNS・リン��</div>
        <div class="sns-buttons">
          <a href="https://www.instagram.com/club_f.k?igsh=MWZteThkajVycW1kdQ==" target="_blank" rel="noopener noreferrer" class="sns-link-btn sns-instagram-btn">📷 Instagram</a>
          <a href="https://www.host2.jp/shop/fk/index_sp.html" target="_blank" rel="noopener noreferrer" class="sns-link-btn sns-hoshos-btn">ホスホス</a>
        </div>
      `;
      ...
      if (items.length === 0 && !editMode) {
        ...
        demoSection.innerHTML = `
          <div class="info-section-header">
            <div class="info-section-title">営業時間</div>
          </div>
        ...
        demoSection2.innerHTML = `
          <div class="info-section-header">
            <div class="info-section-title">電話番号</div>
          </div>
        ...
        items.forEach(item => {
          ...
          section.innerHTML = `
            <div class="info-section-header">
              ...
              ${editMode ? `<button class="info-edit-btn" onclick="openForm('store', ${JSON.stringify(item).replace(/"/g, '&quot;')})">✏️</button>` : ''}
            </div>
    ...
    function openAddressEdit(item) {
      ...
      document.getElementById('formTitle').textContent = '住所・地図を編集';
      document.getElementById('formFields').innerHTML = `
        <div class="form-group">
          <label class="form-label" for="address">住所 *</label>
          <input type="text" class="form-input" id="address" value="${item?.address || '札幌��中央区南5条西6丁目第3エトワールビル5階'}" required>
        </div>
        <div class="form-group">
          <label class="form-label" for="mapUrl">GoogleマップURL *</label>
      ...
      document.getElementById('formButtons').innerHTML = `
        <button type="submit" class="form-btn form-btn-submit" id="submitBtn">保存</button>
        <button type="button" class="form-btn form-btn-cancel" onclick="closeModal()">キャンセル</button>
      `;
    ...
    function renderPrice() {
      ...
      if (items.length === 0 && !editMode) {
        ...
        demoSection.innerHTML = `
          <div class="info-section-header">
            <div class="info-section-title">セット料金</div>
          </div>
          <div class="info-section-content">60分 ¥3,000\n延長30分 ¥1,500</div>
        `;
        content.appendChild(demoSection);
        ...
        demoSection2.innerHTML = `
          <div class="info-section-header">
            <div class="info-section-title">指名料金</div>
          </div>
          <div class="info-section-content">指名料 ¥1,000\n���内指名 ¥500</div>
        `;
        content.appendChild(demoSection2);
      } else {
        items.forEach(item => {
          ...
          section.innerHTML = `
            <div class="info-section-header">
              ...
              ${editMode ? `<button class="info-edit-btn" onclick="openForm('price', ${JSON.stringify(item).replace(/"/g, '&quot;')})">✏️</button>` : ''}
            </div>
    ...
    function renderRecruit() {
      ...
      linkSection.innerHTML = `
        <a href="https://www.host2.jp/shop/fk/recruit_sp.html" target="_blank" rel="noopener noreferrer" class="recruit-detail-btn">📋 求人詳細を見る</a>
      `;
      ...
      contactSection.innerHTML = `
        <a href="https://line.me/ti/p/ogayou1648" target="_blank" rel="noopener noreferrer" class="contact-btn contact-line">💬 LINEで応募</a>
        <a href="tel:08032339498" class="contact-btn contact-tel">📞 電話で応募</a>
      `;
      ...
      if (items.length === 0 && !editMode) {
        ...
        demoSection.innerHTML = `
          <div class="info-section-header">
            <div class="info-section-title">募集内容</div>
          </div>
          <div class="info-section-content">ホストスタッフ募集中！\n未経験者��歓迎\n詳細はお問い合わせください</div>
        `;
        content.appendChild(demoSection);
      } else {
        items.forEach(item => {
          ...
          section.innerHTML = `
            <div class="info-section-header">
              ...
              ${editMode ? `<button class="info-edit-btn" onclick="openForm('recruit', ${JSON.stringify(item).replace(/"/g, '&quot;')})">✏️</button>` : ''}
            </div>
    ...
    function openForm(type, item = null, position = null) {
      ...
      const titles = {
        cast: item ? 'キャストを編集' : '���ャストを追加',
        news: item ? 'ニュースを編集' : 'ニュースを追加',
        album: item ? 'アルバムを編集' : 'アルバムを追加',
        showcase: item ? 'ショーケースを編集' : 'ショーケースを追加',
        store: item ? '店舗情報を編集' : '店舗情報を追加',
        price: item ? '料金情報を編集' : '料金情報を追加',
        recruit: item ? '求人情報を編集' : '求人情報を追���'
      };
      ...
      if (type === 'showcase') {
        ...
        fields.innerHTML = `
          ...
          <div class="form-group">
            <label class="form-label" for="role">役職</label>
            <input type="text" class="form-input" id="role" value="${item?.role || ''}">
          </div>
          <div class="form-group">
            <label class="form-label" for="birthday">誕生日</label>
            <input type="text" class="form-input" id="birthday" value="${item?.birthday || ''}" placeholder="例: 1月1日">
          </div>
          <div class="form-group">
            <label class="form-label" for="mbti">MBTI</label>
            <input type="text" class="form-input" id="mbti" value="${item?.mbti || ''}" placeholder="例: ENFP">
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            <label class="form-label" for="image">プロフィール画像URL</label>
          ...
          <div class="form-group">
            <label class="form-label" for="hoshos">ホスホス URL</label>
        ...
        fields.innerHTML = `
          <div class="form-group">
            <label class="form-label" for="newsTitle">タイトル *</label>
          ...
          <div class="form-group">
            <label class="form-label" for="newsImage">画像URL</label>
        ...
        fields.innerHTML = `
          <div class="form-group">
            <label class="form-label" for="albumTitle">タイトル *</label>
            <input type="text" class="form-input" id="albumTitle" value="${item?.albumTitle || ''}" required>
          </div>
          <div class="form-group">
            <label class="form-label" for="albumCover">カバー画像URL</label>
            <input type="url" class="form-input" id="albumCover" value="${item?.albumCover || ''}">
          </div>
          <div class="form-group">
            <label class="form-label" for="albumImages">アルバム画像URLs（カンマ区切り）*</label>
        ...
        fields.innerHTML = `
          <div class="form-group">
            <label class="form-label" for="storeTitle">タイトル *</label>
            <input type="text" class="form-input" id="storeTitle" value="${item?.storeTitle || ''}" required>
          </div>
          <div class="form-group">
            <label class="form-label" for="storeContent">内容 *</label>
        ...
        fields.innerHTML = `
          <div class="form-group">
            <label class="form-label" for="priceTitle">タイトル *</label>
            <input type="text" class="form-input" id="priceTitle" value="${item?.priceTitle || ''}" required>
          </div>
          <div class="form-group">
            <label class="form-label" for="priceContent">内容 *</label>
        ...
        fields.innerHTML = `
          <div class="form-group">
            <label class="form-label" for="recruitTitle">タイトル *</label>
            <input type="text" class="form-input" id="recruitTitle" value="${item?.recruitTitle || ''}" required>
          </div>
          <div class="form-group">
            <label class="form-label" for="recruitContent">内容 *</label>
      ...
      const buttonsHtml = item && item.__backendId ? `
        <button type="submit" class="form-btn form-btn-submit" id="submitBtn">保存</button>
        <button type="button" class="form-btn form-btn-delete" onclick="handleDelete()">削���</button>
        <button type="button" class="form-btn form-btn-cancel" onclick="closeModal()">キャンセル</button>
      ` : `
        <button type="submit" class="form-btn form-btn-submit" id="submitBtn">保存</button>
        <button type="button" class="form-btn form-btn-cancel" onclick="closeModal()">キャンセル</button>
    ...
    async function handleSubmit(e) {
      ...
      if (result.isOk) {
        closeModal();
        showToast(currentEdit && currentEdit.__backendId ? '更新しました' : '追加しました');
      } else {
